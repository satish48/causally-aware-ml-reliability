...
> Built a production-grade ML observability platform from scratch — beyond accuracy dashboards into actual incident command.
>
> Most ML monitoring tools tell you *that* a model is degrading. This platform answers: **why**, **how fast**, and **what it's costing** — in real time.
>
> Key features:
> - **Causal attribution engine** — ranks deployment events, schema changes, and pipeline anomalies by temporal proximity + feature overlap using exponential decay scoring
> - **Google SRE error budget model** — burn rate alerts (14.4× fast-burn → immediate page) with SLO budget exhaustion forecasting
> - **Financial loss model** — `excess_fraud × 35% leak factor × $85/txn × 600 txn/hr` — not vanity metrics, actual dollar exposure
> - **Two-sample PSI drift detection** — Cohen's d approach gives calibrated per-feature drift scores (primary features ~0.80, stable ~0.03) unlike the PSI=1.00 everywhere antipattern
> - **Live WebSocket dashboard** — causally-ordered incident timeline, deployment posture advisor, and risk forecast — all streaming at 2s push interval
> - **133 tests** — unit + integration covering drift math, causal scoring, SLO burn rate, canary decisions, and full bootstrap payload contracts
>
> Stack: Python 3.12, FastAPI, asyncio pub-sub, Chart.js, Prometheus, Docker, SQLite state store.
>
> What I'd add next: MLflow integration to close the rollback loop, Kafka for horizontal scaling, and per-model-fleet namespacing.
>
> [GitHub link] #MLOps #MachineLearning #Python #FastAPI #ProductionML
